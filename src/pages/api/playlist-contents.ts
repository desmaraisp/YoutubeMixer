...
import { getYoutubePlaylistData } from '@/lib/backend-services/get-youtube-playlist-data'
import { GaxiosError } from 'googleapis-common'
import { createRouter } from "next-connect";
import { OptionalAuthorization } from "@/lib/middleware/auth-middleware";
import { getRefreshTokensFromDB } from "@/lib/firestore/get-set-db-refresh-token";
import { getSpotifyPlaylistData } from "@/lib/backend-services/get-spotify-playlist-data";
...
import { PlaylistContentsRequestSchema } from '../../models/api-models/playlist-contents';
...
router
	...
	.post(
		async (req, res, _next) => {
			...
			const requestPayload = PlaylistContentsRequestSchema.parse(JSON.parse(req.body))
